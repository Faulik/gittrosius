...
function* watchSetUserAppTokenSuccess() {
  while (true) {
    yield take(actions.SET_USER_APP_TOKEN_SUCCESS);
    yield call(fetchUser);
  }
}
...
function* watchUpdateRoom() {
  while (true) {
    const { roomId } = yield take(actions.UPDATE_ROOM);
...
function* watchCheckRoom() {
  while (true) {
    ...
    const joinedRooms = yield select((state) => state.rooms.joined);
    room = joinedRooms.find((elem) => elem.name === name);
    if (room) {
      ...
      if (response) {
        room = response.find((elem) => elem.name === name);
        if (room) {
          yield put(actions.selectRoom(room.id, room.name));
          yield call(fetchRoomMessages, room.id);
        } else {
...
function* watchLoginByOAuth() {
  while (true) {
    const { code } = yield take(actions.LOGIN_USER_BY_OAUTH);
  }
}
